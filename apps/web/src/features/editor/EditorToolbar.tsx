      ...
      <div className="editor-header__primary">
        ...
        <Space size={8}>
          ...
          <Tooltip title={onSave ? "保存看板" : "保存功能将在后续阶段接入"}>
            <Button icon={<SaveOutlined />} aria-label="保存" disabled={!onSave} onClick={onSave}>保存</Button>
          </Tooltip>
          <Tooltip title={onPublish ? "发布看板" : "发布功能将在后续阶段接入"}>
            <Button type="primary" icon={<UploadOutlined />} aria-label="发布" disabled={!onPublish} onClick={onPublish}>发布</Button>
          </Tooltip>
        </Space>
      </div>
