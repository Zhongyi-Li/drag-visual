...
- 日期：2026-07-01
- 状态：2026-07-06 后端路线调整为仓库内自研，待书面复核
- 目标用户：公司内部单组织用户
- 开发资源：1 名前端开发兼后端自研、1 名数据同事协作
...
采用契约优先、仓库内闭环的前后端方案：本仓库交付 React Web、TypeScript 领域契约、OpenAPI、可执行 MSW Mock，以及 `apps/api` 的 NestJS + Prisma + PostgreSQL 后端。

前端仍先基于同一 OpenAPI 契约通过 MSW 开发和测试；真实后端由 `apps/api` 按阶段补齐 Dashboard、Publishing 和 Dataset Gateway，避免跨项目口头对接和二次联调。
...
OpenAPI + 示例 JSON + 稳定错误码
  ...
  └─ 约束 NestJS Controller、DTO 与 Prisma 持久化

NestJS/Prisma Backend
  └─ 在 apps/api 内继续实现真实后端能力
```
...
| Schema 校验 | Zod |
| 前端接口模拟 | MSW |
| 生产 API | NestJS、Fastify、Prisma（`apps/api`） |
| 持久化 | PostgreSQL、Prisma Migrate |
| 接口契约 | OpenAPI |
...
- `openapi/bi-mvp.yaml` 是前后端接口事实源；TypeScript、MSW 与 `apps/api` 字段名、必填性、枚举、状态码必须与其一致。
- `apps/web/src/mocks` 提供 MSW handlers 和固定测试数据，覆盖成功、空数据、超时、字段变化、保存冲突和发布失败。
- `apps/api` 不得要求前端直接调用统一业务接口，也不得向浏览器返回服务端凭据。
- 每个阶段同时更新对应的 `docs/backend-contract/*.md` 和自研后端规划文档；真实后端不阻塞前端 MSW 闭环。
...
- `apps/api` 接入前使用同一 OpenAPI 示例运行契约验收。
...
| 周次 | 重点 | 退出条件 |
| --- | --- | --- |
| 第 1 周 | Workspace、React、Schema、OpenAPI、MSW 与看板 CRUD Mock | 前端可创建并读取 Mock 看板；沉淀 CRUD 契约文档 |
...
| 第 7 周 | 预览、发布快照、重新发布和 `apps/api` 联调 | 发布页与编辑器渲染一致；后端完成发布接口 |
...
每份文档必须包含：NestJS Controller 路由、请求/响应 DTO、字段约束、数据库表或索引、事务与并发规则、稳定错误码、示例 JSON、服务端安全限制和可执行验收用例。实现以 `apps/api` 的模块结构和 Prisma schema 为准。
...
| 真实 API 与前端字段或错误码不一致 | OpenAPI 作为唯一接口契约，契约文档附验收样例 |
| 前端被后端进度阻塞 | 所有阶段先由 MSW 完成，`apps/api` 通过 base URL 切换接入 |

## 18. 交付物

- 可运行的内部 BI 看板 Web 应用。
- 可执行 MSW Mock、固定场景数据和前端 API Client。
- OpenAPI 契约及分阶段 NestJS 后端契约文档。
- `apps/api` NestJS/Prisma 后端实现。
- 六类组件及字段、样式配置。
- 统一业务接口数据链路。
